...
public:
    ...
    // Very used by: menus and hotkeys (buttons, status bar, etc).
    ...
    static ushort GetAltSettings(void) {
        return AltSet;
    }
    static void SetAltSettings(ushort altSet) {
        AltSet = altSet;
    }

    // Special functions when we must deal with international stuff like a
    // shortcut in Greek or Cyrillic.
