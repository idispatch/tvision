/* TDisplay/TScreen header.
 ...
 This idea was proposed by JASC.
...
typedef TScreenFont256 *(*TVScreenFontRequestCallBack)(int which, unsigned w, unsigned height);
...
public:
    ...
    static int setFont(int changeP, TScreenFont256 *fontP, int changeS, TScreenFont256 *fontS,
    ...
    static const char *getDriverShortName() {
        return currentDriverShortName;
    }
    static void beep() {
        TDisplay::beep();
    }
...
protected:
    ...
    static int defaultSystem(const char *command, pid_t *pidChild, int in, int out, int err);
    static int defaultGetFontGeometry(unsigned &w, unsigned &h);
    static int defaultGetFontGeometryRange(unsigned &wmin, unsigned &hmin, unsigned &umax,
            unsigned &hmax);
    static int defaultSetFont(int changeP, TScreenFont256 *fontP, int changeS,
            TScreenFont256 *fontS, int fontCP = -1, int appCP = -1);
