...
#ifndef TVFontCollection_Included
#define TVFontCollection_Included
...
public:
    TVBitmapFontDescLBox(const TRect& bounds, ushort aNumCols, TScrollBar *aHScrollBar,
...
public:
    TVBitmapFontSizeCol() :
            TStringCollection(3, 3) {
    }
    virtual int compare(void *key1, void *key2);
    // do not use the default delete[]
    virtual void freeItem(void *item) {
        delete (TVBitmapFontSize *) item;
    }
};

class TVBitmapFontSizeLBox: public TSortedListBox {
public:
    TVBitmapFontSizeLBox(const TRect& bounds, ushort aNumCols, TScrollBar *aHScrollBar,
            TScrollBar *aVScrollBar, Boolean aCenterOps = False) :
            TSortedListBox(bounds, aNumCols, aHScrollBar, aVScrollBar, aCenterOps) {
    }
    virtual void getText(char *dest, ccIndex item, short maxChars);
...
protected:
    ...
    static void Swap(unsigned *value) {
        Swap((int *) value);
    }
    ;
