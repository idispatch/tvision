...
#if __GNUC__>=3
#if __GNUC_MINOR__>=4 && !defined(__QNXNTO__)
...
/* Return the number of ticks (on MSDOS 1 tick is 1/18 sec),
 this is used to compute the double click */CLY_CFunc unsigned short CLY_Ticks(void);
/* An utility to split directory and file components of a path:
 ...
 The directory will have a trailing slash. */CLY_CFunc void CLY_ExpandPath(const char *path,
        char *dir, char *file);
/* An utility function. It makes the path an absolute one and replaces the
 original value passed. Should use fixpath. */CLY_CFunc void CLY_fexpand(char *rpath);
#undef  fexpand
#define fexpand(a) CLY_fexpand(a)
/* Utility function to know if a drive letter is valid. */CLY_CFunc int CLY_DriveValid(char drive);
#ifdef __cplusplus
#define driveValid(a) (CLY_DriveValid(a) ? True : False)
#endif
/* Utility function to know is a file is in fact a directory */CLY_CFunc int CLY_IsDir(
        const char *str);
#ifdef __cplusplus
#define isDir(a) (CLY_DriveValid(a) ? True : False)
#endif
/* Utility function to know if a path is valid (existing directory or file) */CLY_CFunc int CLY_PathValid(
        const char *path);
#ifdef __cplusplus
#define pathValid(a) (CLY_PathValid(a) ? True : False)
#endif
/* Utility function to know if a file name is valid to create a new file */CLY_CFunc int CLY_ValidFileName(
        const char *fileName);
#ifdef __cplusplus
#define validFileName(a) (CLY_ValidFileName(a) ? True : False)
#endif
/* Utility function to know the current directory including a trailing
 slash */CLY_CFunc void CLY_GetCurDirSlash(char *dir);
#undef  getCurDir
#define getCurDir(a) CLY_GetCurDirSlash(a)
/* Utility function to know if a path contains a supported wildcard */CLY_CFunc int CLY_IsWild(
        const char *f);
#ifdef __cplusplus
#define isWild(a) (CLY_IsWild(a) ? True : False)
#endif
/* Utility function to know if a file exist and we can read from it */CLY_CFunc int CLY_FileExists(
        const char *fname);
#ifndef TVCompf_djgpp
#undef  __file_exists
#define __file_exists(a) CLY_FileExists(a)
#endif
/* Utility function to know if a path is relative */CLY_CFunc int CLY_IsRelativePath(
        const char *path);
#ifdef __cplusplus
...
/* Used internally, just call filelength prior request */CLY_CFunc long CLY_filelength(int);
/* Used internally, just call getcurdir prior request */CLY_CFunc int CLY_getcurdir(int drive,
        char *buffer);
...
/* Returns the name of the shell command */CLY_CFunc char *CLY_GetShellName(void);
