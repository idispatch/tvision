...
static const char *buttonName[] = { __("~Y~es"), __("~N~o"), __("~O~K"), __("Cancel") };

static ushort commands[] = { cmYes, cmNo, cmOK, cmCancel };

static const char *Titles[] = { __("Warning"), __("Error"), __("Information"), __("Confirm") };
...
ushort messageBoxRect(const TRect &r, const char *msg, ushort aOptions) {
    ...
    dialog->insert(new TStaticText(TRect(3, 2, dialog->size.x - 2, height - 3), msg, intlMessage));
    ...
    for (i = 0, x = -2, buttonCount = 0; i < 4; i++) {
        if ((aOptions & (0x0100 << i)) != 0) {
            buttonList[buttonCount] = new TButton(TRect(0, 0, 10, 2), _(buttonName[i]), commands[i],
                    bfNormal);
...
ushort inputBox(const char *Title, const char *aLabel, char *s, int limit) { // Use a size according to the label+limit and title
    int len;
...
ushort inputBoxRect(const TRect &bounds, const char *Title, const char *aLabel, char *s,
        int limit) {
    TDialog *dialog;
    TView* control;
    TRect r;
    ...
    r = TRect(dialog->size.x / 2 - 11, dialog->size.y - 3, dialog->size.x / 2 - 1,
            dialog->size.y - 1);
