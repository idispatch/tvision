...
void CLY_GetFileAttributes(CLY_mode_t *mode, struct stat *statVal, const char *fileName) {
    int ngroups, val;
    gid_t *groups;
    uid_t user;
...
int CLY_SetFileAttributes(CLY_mode_t *newmode, const char *fileName) {
    return !(chmod(fileName, newmode->mode) || chown(fileName, newmode->user, newmode->group));
}
...
void CLY_GetDefaultFileAttr(CLY_mode_t *mode) { // 644, umask will kill the denied ones if that's needed
