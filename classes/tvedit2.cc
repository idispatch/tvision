...
TDialog *createFindDialog() {
    ...
    d->insert(control);
    d->insert(new T1Label(2, 2, __("~T~ext to find"), control));
    d->insert(new THistory(TRect(32, 3, 35, 4), control, 10));

    d->insert(
            new TCheckBoxes(TRect(3, 5, 35, 7),
                    new TSItem(__("~C~ase sensitive"), new TSItem(__("~W~hole words only"), 0))));

    d->insert(new TButton(TRect(14, 9, 24, 11), __("O~K~"), cmOK, bfDefault));
    d->insert(new TButton(TRect(26, 9, 36, 11), __("Cancel"), cmCancel, bfNormal));
...
TDialog *createReplaceDialog() {
    ...
    d->insert(
            new TCheckBoxes(TRect(3, 8, 37, 12),
                    new TSItem(__("~C~ase sensitive"),
                            new TSItem(__("~W~hole words only"),
                                    new TSItem(__("~P~rompt on replace"),
                                            new TSItem(__("~R~eplace all"), 0))))));

    d->insert(new TButton(TRect(17, 13, 27, 15), __("O~K~"), cmOK, bfDefault));
    d->insert(new TButton(TRect(28, 13, 38, 15), __("Cancel"), cmCancel, bfNormal));
