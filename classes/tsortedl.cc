...
TSortedListBox::TSortedListBox(const TRect& bounds, ushort aNumCols, TScrollBar *aScrollBar) :
        TListBox(bounds, aNumCols, aScrollBar), shiftState(0), searchPos(USHRT_MAX) {
    showCursor();
    setCursor(1, 0);
}

// SET:
TSortedListBox::TSortedListBox(const TRect& bounds, ushort aNumCols, TScrollBar *aHScrollBar,
        TScrollBar *aVScrollBar, Boolean aCenterOps) :
        TListBox(bounds, aNumCols, aHScrollBar, aVScrollBar, aCenterOps), shiftState(0), searchPos(
                USHRT_MAX) {
    showCursor();
    setCursor(1, 0);
}
...
void TSortedListBox::handleEvent(TEvent& event) {
    ...
    if (event.what == evKeyDown) {
        if (event.keyDown.keyCode != kbEnter
                && (event.keyDown.charScan.charCode != 0 || event.keyDown.keyCode == kbBack)) {
            value = focused;
            if (value < range)
            ...
            if (event.keyDown.keyCode == kbBack) {
                ...
                char *loc = strchr(curString + (searchPos == USHRT_MAX ? 0 : searchPos), '.');
                if (loc) {
            ...
            if (value < range) {
                ...
                searchPos = oldPos;
            if (searchPos != oldPos || ucisalpha( event.keyDown.charScan.charCode ))
                clearEvent(event);
        }
    }
}
