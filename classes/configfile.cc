...
const char *TVConfigFile::Errors[] = { __("Can't open file"), // -1
...
/**[txh]********************************************************************
 ...
 Description:
 Searches for the indicated key in the tree. Separate nested sections using
 forward slashes.
...
TVConfigFileTreeNode *TVConfigFile::SearchOnlyInBranch(TVConfigFileTreeNode *b, char *name,
        int len) {
    if (!b)
        return NULL;
...
int TVConfigFile::AddString(const char *key, const char *name, const char *value, int priority) {
    TVConfigFileTreeNode *node = new TVConfigFileTreeNode;
    node->type = tString;
    node->priority = priority;
...
int TVMainConfigFile::Add(const char *section, const char *name, const char *value) {
    int lSection = section ? strlen(section) + 1 : 0;
    AllocLocalStr(b, lSection+4);
    strcpy(b, "TV");
    if (section) {
