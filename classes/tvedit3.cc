...
TMenuBar *TEditorApp::initMenuBar(TRect r) {
    ...
    TSubMenu& sub1 = *new TSubMenu(__("~F~ile"), kbAltF)
            ...
            + *new TMenuItem(__("S~a~ve as..."), cmSaveAs, kbNoKey) + newLine()
    ...
    TSubMenu& sub2 = *new TSubMenu(__("~E~dit"), kbAltE)
            + *new TMenuItem(__("~U~ndo"), cmUndo, kbNoKey) + newLine()
            ...
            + *new TMenuItem(__("~S~how clipboard"), cmShowClip, kbNoKey) + newLine()
...
ushort doEditDialogDefault(int dialog, va_list arg) {
    ...
    case edSaveAs: {
        return execDialog(new TFileDialog("*", __("Save file as"), __("~N~ame"), fdOKButton, 101),
                va_arg( arg, _charPtr ));
    }
...
ushort doEditDialog(int dialog, ...) { // Just call the default
    va_list arg;
