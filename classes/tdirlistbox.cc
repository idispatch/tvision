...
TDirListBox::TDirListBox(const TRect& bounds, TScrollBar *aVScrollBar, TScrollBar *aHScrollBar) :
        TListBox(bounds, 1, aHScrollBar, aVScrollBar), cur(0) {
    *dir = EOS;
    incPos = 0;
...
void TDirListBox::handleEvent(TEvent& event) {
    if (event.what == evMouseDown && event.mouse.doubleClick) {
        ...
        clearEvent(event);
        return;
    } else if (event.what == evKeyDown) { // SET: Incremental directory search
            ...
            if (focused < c && cur < c) {
                ...
                if (focused > cur)
                    ...
                    for (int i = focused; i < c; i++) {
                        if (strncasecmp(dirs->at(i)->dir() + offset, incremental, incPos) == 0) {
                            newFocus = i;
                            break;
                        }
                    }
...
static TStringCollection *ListDirectory(char *path, char *end) {
    *end = 0;
