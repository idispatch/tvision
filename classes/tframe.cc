...
void TFrame::handleEvent(TEvent& event) {
    ...
    if ((event.what & (evMouseDown | evMouseUp)) && (state & sfActive) != 0) {
        ...
        if (mouse.y == 0) {
            if ((((TWindow *) owner)->flags & wfClose) != 0 && mouse.x >= 2 && mouse.x <= 4) {
                ...
                if (event.mouse.doubleClick
                        || ((((TWindow *) owner)->flags & wfZoom) != 0 && mouse.x >= size.x - 5
                                && mouse.x <= size.x - 3)) {
                    ...
                    if ((((TWindow *) owner)->flags & wfMove) != 0 && event.what == evMouseDown) {
                        dragWindow(event, dmDragMove);
                    }
                }
            }
        } else {
            if (mouse.x >= size.x - 2 && mouse.y >= size.y - 1 && event.what == evMouseDown) {
                if ((((TWindow *) owner)->flags & wfGrow) != 0) {
...
void TFrame::frameLine(TDrawBuffer& frameBuf, short y, short n, uchar color) {
    ...
    cx = di - si;
    while (cx--)
        FrameMask[si++] |= (((ax & 0xff00) >> 8) & 0x00ff);
    goto lab1;
    lab10: dx++;
    {
