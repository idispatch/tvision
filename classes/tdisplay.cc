...
TVScreenDriverDetectCallBack TDisplay::screen_driver_detect_callback = NULL;
...
Boolean TDisplay::searchDOSModeInfo(ushort mode, unsigned &w, unsigned &h, int &fW, int &fH) {
    int i;
...
/**[txh]********************************************************************
 Description: Selects the mode that's closest to the specified width and
...
int TDisplay::defaultSetCrtModeRes(unsigned /*w*/, unsigned /*h*/, int /*fW*/, int /*fH*/) {
    return 0;
}
...
Boolean TDisplay::searchClosestRes(TScreenResolution *res, unsigned x, unsigned y, unsigned cant,
        unsigned &pos) {
    ...
    for (i = 0; i < cant && res[i].x < x; i++) {
        if (res[i].x == x) {
            if (firstXMatch == -1)
                firstXMatch = i;
            if (res[i].y == y) { // Exact match
                pos = i;
                return True;
            }
        }
    }
    if (firstXMatch != -1) { // Return the closest y that match x
...
TVScreenDriverDetectCallBack TDisplay::setDetectCallBack(TVScreenDriverDetectCallBack aCB) {
    TVScreenDriverDetectCallBack ret = screen_driver_detect_callback;
    screen_driver_detect_callback = aCB;
    return ret;
}
