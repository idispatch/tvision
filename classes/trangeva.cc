...
TRangeValidator::TRangeValidator(long aMin, long aMax) :
    ...
    Max = aMax;
    if (Min >= 0)
        strcpy(ValidChars, ctValidCharsPos);
    if (Min < 0 && Max < 0)
        strcpy(ValidChars, ctValidCharsNeg);
}
...
void TRangeValidator::Error() {
    if (Min >= 0)
        ...
        messageBox(mfError | mfOKButton, __("Value not in the range %li to %li"), Min, Max);
    // SET: Replaced %U and %D because MSVC lacks it
}

static long get_val(const char *buf) { // SET: Changed to use strtol
    char *end;
    return strtol(buf, &end, 0);
}

static unsigned long get_uval(const char *buf) { // SET: Changed to use strtoul
    char *end;
...
Boolean TRangeValidator::IsValid(const char *S) {
    long value;
    if (TFilterValidator::IsValid(S) == False)
        return False;
    if (Min >= 0)
        value = get_uval(S);
    else
