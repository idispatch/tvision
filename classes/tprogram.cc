...
void TProgram::getEvent(TEvent& event) {
    ...
    if (statusLine != 0) {
        if ((event.what & evKeyDown) != 0
                || ((event.what & evMouseDown) != 0 && firstThat(hasMouse, &event) == statusLine))
    ...
    if (event.what == evKeyDown) {
        ...
        if (c >= '1' && c <= '9') {
            if (current->valid(cmReleasedFocus)) {
                if (message(deskTop, evBroadcast, cmSelectWindowNum, (void *) (long) (c - '0'))
                        != 0)
                    clearEvent(event);
            }
        }
...
TStatusLine *TProgram::initStatusLine(TRect r) {
    ...
    return new TStatusLine(r,
            *new TStatusDef(0, 0xFFFF) + *new TStatusItem(__("~Alt-X~ Exit"), kbAltX, cmQuit)
