...
static uint32 masks[32] = { 0x00000001, 0x00000002, 0x00000004, 0x00000008, 0x00000010, 0x00000020,
        0x00000040, 0x00000080, 0x00000100, 0x00000200, 0x00000400, 0x00000800, 0x00001000,
...
static inline uint32 mask(int cmd) {
    return masks[cmd & 0x1F];
}

static inline uint32 rest(int cmd) {
    return cmd & 0x1F;
}
...
Boolean TCommandSet::has(int cmd) {
    return Boolean((cmd >= MAX_COMMANDS) || (cmds[loc(cmd)] & mask(cmd)) != 0);
}
