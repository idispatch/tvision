...
Boolean call10(const TEditor *edit, ushort *drawBuf, ushort color, int cx, int &offset,
        unsigned &lineptr, int &bufptr, int Width) {
    uchar c;
...
void TEditor::formatLine(void *DrawBuf, uint32 LinePtr, int Width, ushort Colors) {
    ...
    if (count < 1)
        return;
    while (count--)
        ((ushort *) (DrawBuf))[bufptr++] = endianCol(' ', color);
}
