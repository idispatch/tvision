...
public:
    ...
    stTVIntl *atI(ccIndex item) {
        return (stTVIntl *) at(item);
    }
};
...
TCluster::TCluster(const TRect& bounds, TSItem *aStrings) :
    ...
    options |= ofSelectable | ofFirstClick | ofPreProcess | ofPostProcess | extraOptions;
    int i = 0;
    TSItem *p;
...
void TCluster::drawBox(const char *icon, char marker) {
    ...
    for (int i = 0; i <= size.y; i++) {
        for (int j = 0; j <= (strings->getCount() - 1) / size.y + 1; j++) {
            int cur = j * size.y + i;
            int col = column(cur);
            if ((cur < strings->getCount()) && (col + cstrlen(getItemText(cur)) + 5 < maxViewWidth)
                    && (col < size.x)) {
            ...
            for (int i = 0; i < strings->getCount(); i++) {
                ...
                if (TGKey::GetAltCode(c) == event.keyDown.keyCode
                        || ((owner->phase == phPostProcess || (state & sfFocused) != 0) && c != 0
                                && TGKey::CompareASCII(uctoupper(event.keyDown.charScan.charCode),
                                        c))) {
                    select();
                    sel = i;
                    movedTo(sel);
