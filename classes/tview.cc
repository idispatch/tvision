...
TView::TView(const TRect& bounds) :
        next(0), options(0), eventMask(evMouseDown | evKeyDown | evCommand), state(sfVisible), growMode(
                0), dragMode(dmLimitLoY), helpCtx(hcNoContext), owner(0) {
    setBounds(bounds);
    cursor.x = cursor.y = 0;
}
...
void TView::resetCursor() {
    ...
    if (((~state) & (sfVisible /*| sfCursorVis*/| sfFocused)) == 0
        ...
        while (lookNext && ((x >= 0) && (x < target->size.x)) && ((y >= 0) && (y < target->size.y))) {
            y += target->origin.y;
            x += target->origin.x;
            if (!target->owner) { // Target is the one connected to the screen, set the screen cursor
            ...
            do {
                view = view->next;
                if (view == target) { // Ok x,y is inside target and nobody is over it.
                    target = view->owner;
                    lookNext = 1;
                    break;
                }
            } while (!(view->state & sfVisible) || y < view->origin.y
                    || y >= view->origin.y + view->size.y || x < view->origin.x
...
Boolean lineExposed(TView *view, int line, int x1, int x2, TView *target) {
    ...
    while (1) {
        if (!target) { // This is a call to start searching, we must initialize
            target = view;
        ...
        while (1) {
            view = view->next;
            if (view == target) { // No other TView is overlapping us
            ...
            Xtest += view->size.x;
            if (x2 <= Xtest) { // Reduce the x range
                x2 = view->origin.x;
                continue;
            }
...
static
void blitBuffer(TView *view, int line, int xStart, int xEnd, int offset, const ushort *buffer,
        int inShadow) {
    ...
    if (TDisplay::getDrawingMode() == TDisplay::unicode16) { // The display uses Unicode
        ...
        if (isScreen && TVCodePage::OnTheFlyRemapNeeded()) {
            ...
            toBlit = (const ushort *) aux;
        } else { // We don't need to remap, but ...
            if (inShadow) { // is much more efficient to call the OS just once
                memcpy(aux, toBlit, count * sizeof(ushort));
                int i;
                for (i = 0; i < count; i++) {
...
static
void WriteView(int xStart, int line, int xEnd, const void *buffer, TView *view, int offset,
        int inShadow, TView *target) {
    int x, y, skipInit = 0;

    if (!target) { // Initial call so initialize
                   // Check line is valid
        if (line < 0 || line >= view->size.y)
            return;
        // Validate x range
        if (xStart < 0)
            xStart = 0;
    ...
    do {
        if (skipInit)
            skipInit = 0;
        else { // Pass to the owner or init if that's the first call
            if (!(view->state & sfVisible) || !view->owner)
                return;
        ...
        do {
            ...
            if (line < y)
                continue;
            y += view->size.y;
            if (line >= y) { // The line is outside, now check for the shadow
                ...
                if (!(view->state & sfShadow))
                    continue;
                // Now add the shadow
                if (line < view->origin.y + shadowSize.y)
                    continue;
                x += shadowSize.x;
            }
            // This part deals with the part that can be under the shadow
            if (xStart >= x)
                continue; // No in shadow
            inShadow++;
            if (xEnd <= x)
                continue; // Full in shadow
        ...
        if (!owner->buffer) { // and locked avoid drawing
            if (owner->lockFlag)
                return;
            // else go deeper
            continue;
        ...
        if (line != TEventQueue::curMouse.where.y || xStart > TEventQueue::curMouse.where.x
                || xEnd <= TEventQueue::curMouse.where.x) { // The mouse is not in the draw area
            TMouse::resetDrawCounter();
            blitBuffer(view, line, xStart, xEnd, offset, ((const ushort *) buffer), inShadow);
            if (TMouse::getDrawCounter() == 0) { // There was no mouse event
                if (owner->lockFlag)
                    return;
                continue;
            }
...
void TView::writeBuf(int x, int y, int w, int h, TDrawBufferBase& b) {
    ...
    if (TDisplay::getDrawingMode() == TDisplay::codepage) { // Buffer is unicode and screen codepage
...
void TView::writeBuf(int x, int y, int w, int h, const void *Buffer) {
    if (TDisplay::getDrawingMode() == TDisplay::codepage) { // The buffer is in native mode
...
void TView::writeCharU16(int x, int y, unsigned c, unsigned color, int count) {
    if (count <= 0)
        return;
    if (TDisplay::getDrawingMode() == TDisplay::codepage) { // Not in native mode
