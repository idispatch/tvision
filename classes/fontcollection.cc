...
void TVFontCollection::ReduceOne(uchar *dest, uchar *ori, int height, int wBytes, int num) {
    int i;
    ...
    for (i = 0; i < num; i++) {
        if (isWordChar(i) && ori[0]) { // first pixel is used
...
void TVFontCollection::EnlargeOne(uchar *dest, uchar *ori, int height, int wBytes, int num) {
    int i;
    unsigned sizeDest = wBytes * height;
    unsigned sizeOri = sizeDest - wBytes;
...
uchar *TVFontCollection::GetFontFull(int width, int height, int &first, int &last) {
    int oneMore = 0, oneLess = 0;
    SizeFont sz = { width, height };
...
TVBitmapFontDescCol *TVFontCollection::CreateListOfFonts(const char *dir, unsigned wmin,
