...
TButton::TButton(const TRect& bounds, const char *aTitle, ushort aCommand, ushort aFlags) :
        TView(bounds), title(newStr(aTitle)), intlTitle(NULL), command(aCommand), flags(aFlags), amDefault(
                Boolean((aFlags & bfDefault) != 0)) {
...
void TButton::drawTitle(TDrawBuffer &b, int s, int i, ushort cButton, Boolean down) {
    int l, scOff;
