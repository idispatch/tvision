    ...
    if (!f) {
        ...
        if (setBufSize((uint32) (fSize)) == False) {
            ...
            if (fSize > INT_MAX) {
                ...
                fread(&buffer[bufSize - (uint32) (fSize) + INT_MAX], (uint32) (fSize - INT_MAX), 1,
                        f);
...
Boolean TFileEditor::saveFile() {
    ...
    if ((editorFlags & efBackupFiles) && edTestForFile(fileName)) // Forget about it if that's a new file
            {
        char *dot, *slash;
